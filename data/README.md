...
└── market/                      # 行情 (CSV)
    └── <symbol>/
        ...
        ├── daily_hfq.csv        # 日线·后复权(乘法复权,长区间不为负)
        └── daily_raw.csv        # 日线·不复权(真实历史价,算股息率/估值用)
```

> **三种复权的用途**:前复权(qfq)便于看近期走势但历史绝对价被扭曲(高分红股长区间甚至为负);
> 后复权(hfq)适合算长期收益率;**不复权(raw)是真实成交价**,用于点位类计算(股息率、PE 等)。
> 复权数值均由 AKShare/东财算好透传,本项目不自行计算。
>
> **当年末股息率** = 当年每股分红 / 当年末不复权收盘(分子分母同为当年股本口径,跨拆股仍正确),
> 由前端 `buffett-web` 实时计算展示。
